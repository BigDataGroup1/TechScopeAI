"""Web search utility for fallback when RAG doesn't find sufficient results."""
...
# Try to import web search libraries (try new package name first, then old)
try:
...
class WebSearcher:
    """Web search utility with topic relevance filtering."""
    ...
    def __init__(self, max_results: int = 5, min_relevance_score: float = 0.3):
        ...
        if DDGS_AVAILABLE:
            try:
                self.ddgs = DDGS()
                logger.info("✅ Web search initialized (DuckDuckGo)")
    ...
    def search(self, query: str, topic_context: Optional[str] = None, 
        ...
        Returns:
            List of search results with title, snippet, url, and relevance_score
        """
        if not self.ddgs:
        ...
        try:
            ...
            if not results:
                logger.info(f"No web search results for: {query}")
                return []
    ...
    def _calculate_relevance(self, result: Dict, query: str, 
        ...
        if not query_terms:
            return 0.5  # Default relevance if no meaningful terms
