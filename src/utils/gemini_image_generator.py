            ...
            try:
                ...
                for model_name in image_models_to_try:
                    try:
                        ...
                        # First, try to get the raw response parts directly
                        try:
                            # Check if response has a direct way to get images
                            if hasattr(response, 'candidates') and response.candidates:
                                for candidate in response.candidates:
                                    # Try to access content.parts directly
                                                        ...
                                                        if isinstance(raw_data, str):
                                                            try:
                                                                ...
                                                                if len(image_data) > 1000:
                                                                    logger.info(f"✅ Extracted image from inline_data.data (string), size: {len(image_data)} bytes")
                                                                    break
                                                            except:
                                                                pass
                                                            ...
                                                            image_data = raw_data
                                                            if len(image_data) > 1000:
                                                                logger.info(f"✅ Extracted image from inline_data.data (bytes), size: {len(image_data)} bytes")
                                                                break
                                                # Also check for direct data attribute
                                                if hasattr(part, 'data'):
                                                    part_data = part.data
                                                    if isinstance(part_data, bytes) and len(part_data) > 1000:
                                                        image_data = part_data
                                                        logger.info(f"✅ Extracted image from part.data, size: {len(image_data)} bytes")
                                                        break
                                        if image_data and len(image_data) > 1000:
                                            break
                        except Exception as e:
                            logger.debug(f"Error in initial extraction attempt: {e}")
                        
                        # If still no image, try the existing extraction logic
                        if not image_data or len(image_data) <= 1000:
                            # Try candidates.content.parts first (as suggested by the warning)
                            if hasattr(response, 'candidates') and response.candidates:
                                for candidate in response.candidates:
                                            ...
                                            for part in candidate.content.parts:
                                                    ...
                                                    if hasattr(part.inline_data, 'data') and part.inline_data.data:
                                                        try:
                                                            ...
                                                            if isinstance(data_str, str):
                                                                ...
                                                                # Might already be decoded
                                                                image_data = data_str
                                                            else:
                                                                continue
                                                            
                                                            if len(image_data) > 1000:  # Valid image should be > 1KB
                                                                logger.info(f"✅ Found image in candidates.content.parts.inline_data, size: {len(image_data)} bytes")
                                                                break
                                                            else:
                                                ...
                                                # Check for direct data attribute
                                                    ...
                                                    if isinstance(data, bytes) and len(data) > 1000:
                                                        image_data = data
                                                        logger.info(f"✅ Found image in candidates.content.parts.data, size: {len(image_data)} bytes")
                                                        break
                                                        ...
                                                        try:
                                                            ...
                                                            if len(decoded) > 1000:
                                                                image_data = decoded
                                                                logger.info(f"✅ Decoded base64 image from candidates.content.parts, size: {len(image_data)} bytes")
                                                                break
                                                        except:
                                                            pass
                                            if image_data and len(image_data) > 1000:
                            ...
                            # Fallback to response.parts if candidates didn't work
                            if (not image_data or len(image_data) <= 1000) and hasattr(response, 'parts') and response.parts and len(response.parts) > 0:
                                for part in response.parts:
                                    ...
                                    if hasattr(part, 'inline_data') and part.inline_data:
                                        if hasattr(part.inline_data, 'data') and part.inline_data.data:
                                            try:
                                                ...
                                                if len(image_data) > 1000:  # Valid image should be > 1KB
                                                    logger.info(f"✅ Found image in response.parts.inline_data, size: {len(image_data)} bytes")
                                                    break
                                                else:
                                                    logger.warning(f"Image data too small: {len(image_data)} bytes")
                                            except Exception as e:
                                    ...
                                    # Check for direct data
                                        ...
                                        if isinstance(data, bytes) and len(data) > 1000:
                                            image_data = data
                                            logger.info(f"✅ Found image in response.parts.data, size: {len(image_data)} bytes")
                                            break
                                        elif isinstance(data, str):
                                            # Might be base64 encoded
                                            try:
                                                ...
                                                if len(decoded) > 1000:
                                                    image_data = decoded
                                                    logger.info(f"✅ Decoded base64 image from response.parts, size: {len(image_data)} bytes")
                                                    break
                                            except:
                                                pass
                        ...
                        if image_data and len(image_data) > 1000:  # Minimum 1KB for valid image
                            ...
                            # Log response structure for debugging (but don't print binary data)
                            logger.info(f"🔍 Debugging response structure from {model_name}:")
                            if hasattr(response, 'candidates') and response.candidates:
                                logger.info(f"  Response has {len(response.candidates)} candidates")
                                for i, candidate in enumerate(response.candidates):
                                    logger.info(f"  Candidate {i}:")
                                    if hasattr(candidate, 'content'):
                                        logger.info(f"    Has content: {type(candidate.content).__name__}")
                                        if hasattr(candidate.content, 'parts'):
                                            ...
                                            for j, part in enumerate(candidate.content.parts):
                                                ...
                                                logger.info(f"      Part {j} attributes: {attrs}")
                                                if hasattr(part, 'inline_data'):
                                                    inline_data = part.inline_data
                                                    logger.info(f"      Part {j} has inline_data: {type(inline_data).__name__}")
                                                    if hasattr(inline_data, 'data'):
                                                        data = inline_data.data
                                                        if isinstance(data, bytes):
                                                            logger.info(f"        Data type: bytes, size: {len(data)} bytes")
                                                        elif isinstance(data, str):
                                                            logger.info(f"        Data type: str, length: {len(data)} chars")
                                                        else:
                                                            logger.info(f"        Data type: {type(data).__name__}")
                            if hasattr(response, 'text'):
                                text_preview = str(response.text)[:200] if response.text else "None"
                                logger.info(f"  Response text (first 200 chars): {text_preview}")
                            if hasattr(response, 'parts'):
                                logger.info(f"  Response has {len(response.parts)} parts (direct)")
                                for i, part in enumerate(response.parts):
                                    attrs = [attr for attr in dir(part) if not attr.startswith('_')]
                                    logger.info(f"    Part {i} attributes: {attrs[:10]}")
